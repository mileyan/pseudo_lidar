...
def generate_disparity_from_velo2(pc_velo, height, width, calib):
    pts_2d_L = calib.project_velo_to_image(pc_velo)
    pts_2d_R = calib.project_velo_to_imageR(pc_velo)
    disp_all = pts_2d_L - pts_2d_R
    fov_inds = (pts_2d_L[:, 0] < width - 1) & (pts_2d_L[:, 0] >= 0) & \
               (pts_2d_L[:, 1] < height - 1) & (pts_2d_L[:, 1] >= 0)
    fov_inds = fov_inds & (pc_velo[:, 0] > 2)
    imgfov_pts_2d = pts_2d_L[fov_inds, :]
    imgfov_pts_2d = np.round(imgfov_pts_2d).astype(int)
    disp_fov = disp_all[fov_inds, :]
    disp_map = np.zeros((height, width)) - 5 
    for i in range(imgfov_pts_2d.shape[0]):
        disp_map[int(imgfov_pts_2d[i, 1]), int(imgfov_pts_2d[i, 0])] = disp_fov[i, 0]
    return disp_map


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate Disparity')
    parser.add_argument('--data_path', type=str, default='/Users/mengsli/Downloads/DLRepo/KITTI/object/training/')
    parser.add_argument('--split_file', type=str, default='/Users/mengsli/Downloads/DLRepo/KITTI/object/training/train.txt')
    ...
    for fn in lidar_files:
        ...
        disp2 = generate_disparity_from_velo2(lidar, height, width, calib)

        np.save(disparity_dir + '/' + predix, disp)
        np.save(disparity_dir + '/' + predix + '_2', disp2)
